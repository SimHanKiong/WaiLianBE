...
def read_students(db: Session) -> list[StudentOutWithParent]:
    students = student_crud.read_all(db)
    return [StudentOutWithParent.model_validate(student) for student in students]


def update_student(
    db: Session, id: UUID, student_in: StudentUpdate
) -> StudentOutWithParent | None:
    student = student_crud.update(db, id, student_in)
    if not student:
        return None
    return StudentOutWithParent.model_validate(student)


def delete_students(db: Session, ids: list[UUID]) -> list[StudentOutWithParent]:
